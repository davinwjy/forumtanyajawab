  ...
  <ul class="nav navbar-nav navbar-right">
      <li class="dropdown active"><a class="dropdown-toggle" data-toggle="dropdown" href="#"><span class="glyphicon glyphicon-user"></span><?php echo " ".$login_session ?></a>
        <ul class="dropdown-menu">   
          ...
          <li class="active"><a href="usercomment.php?id=<?php echo $fetch["id"]; ?>">
            Forum Comment
          </a></li>           
        </ul>
      </li>      
...
<?php
  ...
  $sqljoin="select tb_comment.id as id_comment,tb_comment.isi,tb_thread.judul,tb_thread.id as id_thread,tb_comment.created_at,tb_thread.idmatpel,tb_matpel.namamatpel from tb_comment 
  join tb_thread on tb_comment.idthread = tb_thread.id 
  join tb_matpel on tb_thread.idmatpel = tb_matpel.id where tb_comment.username='$username'";
...
?>
      <div class="row">
          <div class="col-sm-5">
            <h3 style="font-weight: bold;">Forum Comment</h3>
          </div>
          <div class="col-sm-7"  style="text-align: right; padding-right: 50px;">            
            <ul class="pagination">
              ...
              for ($i = 1; $i <= $pages; $i++)
                {
              ?>
                <li id="<?php echo $i;?>"><a href="usercomment.php?id=<?php echo $_GET['id'];?>&page=<?php echo $i;?>"><?php echo $i;?></a></li>
              <?php           
                }
              ?>
            </ul>
                ...
                <table class="table table-striped" border="2px">
                    <th width="100%">
                        <div class="row">
                            <div class="col-sm-4" style="padding-top: 5px;">
                                <?php 
                                  echo $fetchjoin['created_at']."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"; 
                                ?>
                            </div>
                            <div class="col-sm-3" style="padding-top: 5px;">
                                <a href="formmatpel.php?idmatpel=<?php echo $fetchjoin['idmatpel']; ?>" style="color:black">
                                <?php
                                  echo $fetchjoin['namamatpel']."</a>";
                                ?>
                            </div>
                            <div class="col-sm-5" style="text-align: right;">
                              <?php
                                  ...
                                  if($login_session==$username){
                              ?>      
                              <a href="#" class="btn btn-default"  onclick="hapus(<?php echo $fetchjoin['id_comment']; ?>);return false;">Delete</a>
                              <a href="edit_comment.php?idthread=<?php echo $fetchjoin['id_thread']; ?>&id=<?php echo $fetchjoin['id_comment']; ?>" class="btn btn-default" >Edit</a>
                              <?php
                    ...
                    <tr>
                        <td style="word-wrap: break-word;min-width: 160px;max-width: 200px;">
                          <a href="formthread.php?id=<?php echo $fetchjoin['id_thread']; ?>" style="color:black;font-weight: bold;">
                          <?php echo $fetchjoin['judul']."</a><br>"; 
                          $text=$fetchjoin['isi'];
                          $text=strip_tags($text,"<img>");
                          $text = htmlspecialchars($text, ENT_QUOTES);
                          $turned =array( 'assets/user','&lt;img',';&quot;&gt;');
                          $turn_back=array( '',' &lt;img',';&quot;&gt; ');
                          $text=str_replace($turned, $turn_back, $text);
                          echo $text;
                          // $turned = array( '&lt;p&gt;', '&lt;/p&gt;', '&lt;b&gt;', '&lt;/b&gt;', '&lt;em&gt;', '&lt;/em&gt;', '&lt;u&gt;', '&lt;/u&gt;', '&lt;ul&gt;', '&lt;/ul&gt;', '&lt;li&gt;', '&lt;/li&gt;', '&lt;ol&gt;', '&lt;/ol&gt;', '&lt;br&gt;','assets/user/','&lt;img');
                          // $turn_back = array( '', '', '<b>', '</b>', '<em>', '</em>', '<u>', '</u>', '<ul>', '</ul>', '<li>', '</li>', '<ol>', '</ol>' ,'','',' &lt;img');
                    ...
                    ?>
                        <br><center>                    
                        <h2 style="padding: 60px;">User ini belum pernah comment</h2></center>
                    <?php
                        }   
                    ?>
                </table>
...
<script type="text/javascript">
  ...
  function hapus(id){
    swal("Anda ingin hapus komentar ini?", {
    buttons: {
    cancel: "Cancel",
    okay: true,
    },
